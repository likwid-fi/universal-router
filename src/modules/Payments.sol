...
abstract contract Payments is RouterImmutables {
    ...
    /// @notice Pays an amount of ETH or ERC20 from a sender to a recipient
    /// @param token The token to pay (can be ETH using Constants.ETH)
    /// @param sender The address that will send the payment
    /// @param recipient The address that will receive the payment
    /// @param value The amount to pay (can be CONTRACT_BALANCE)
    /// @dev If the sender is the contract itself, it will pay from its own balance
    /// @dev If the token is ETH, it will transfer ETH directly
