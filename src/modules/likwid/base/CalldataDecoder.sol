...
library CalldataDecoder {
    ...
    /// @dev equivalent to: abi.decode(params, (ILikwidV2Router.ExactInputParamsLikwidV2))
    function decodeSwapExactInParams(bytes calldata params)
        internal
        pure
        returns (ILikwidV2Router.ExactInputParamsLikwidV2 calldata swapParams)
    {
        // ExactInputParamsLikwidV2 is a variable length struct so we just have to look up its location
        assembly ("memory-safe") {
    ...
    /// @dev equivalent to: abi.decode(params, (ILikwidV2Router.ExactInputSingleParamsLikwidV2))
    function decodeSwapExactInSingleParams(bytes calldata params)
        internal
        pure
        returns (ILikwidV2Router.ExactInputSingleParamsLikwidV2 calldata swapParams)
    {
        // ExactInputSingleParamsLikwidV2 is a static struct
        assembly ("memory-safe") {
            // It is 5 slots (160 bytes)
            if lt(params.length, 0xA0) {
                mstore(0, SLICE_ERROR_SELECTOR)
                revert(0x1c, 4)
            }
            // The params are the struct, not a pointer to it.
            swapParams := params.offset
        }
    }

    /// @dev equivalent to: abi.decode(params, (ILikwidV2Router.ExactOutputParamsLikwidV2))
    function decodeSwapExactOutParams(bytes calldata params)
        internal
        pure
        returns (ILikwidV2Router.ExactOutputParamsLikwidV2 calldata swapParams)
    {
        // ExactOutputParamsLikwidV2 is a variable length struct so we just have to look up its location
        assembly ("memory-safe") {
            // only safety checks for the minimum length, where path is empty
            // 0xa0 = 5 * 0x20 -> 3 elements, path offset, and path length 0
            if lt(params.length, 0xa0) {
    ...
    /// @dev equivalent to: abi.decode(params, (ILikwidV2Router.ExactOutputSingleParamsLikwidV2))
    function decodeSwapExactOutSingleParams(bytes calldata params)
        internal
        pure
        returns (ILikwidV2Router.ExactOutputSingleParamsLikwidV2 calldata swapParams)
    {
        // ExactOutputSingleParamsLikwidV2 is a static struct
        assembly ("memory-safe") {
            // It is 5 slots (160 bytes)
            if lt(params.length, 0xA0) {
                mstore(0, SLICE_ERROR_SELECTOR)
